...
for f in files:
    ...
    for i in range (0,numquestions):
        ...
        if ('Z' in correctanswer[i]):
            numuseful -= 1;

    questionstoday[day] = numuseful;    
    print correctanswer    
...
#        thisstudentrecord = students[thisstudent];
        thisstudentcorrect = students[thisstudent];
        thisstudentseen = dseen[thisstudent];        
...
#            print f,i,thisstudent,thisanswer,correctanswer[i],result;
 ...
 #           thisstudentrecord[qkey[i]] = result; # add new q-records to student 
            if (result): 
                numrighttoday = numrighttoday + 1;
            

        thisstudentcorrect[day] = numrighttoday; # how many they got right today
        students[thisstudent] = thisstudentcorrect; # refile whole set of student q-records with student 
    csvfile.close();

csvout = open(examsdir+"clickergrades.csv","w")
mywriter = csv.writer(csvout);

mywriter.writerow(["name","clicker","ID number","correct","seen","unusedabs"])
...
for individual in listofstudents:
    ...
    thisstudentcorrect = students[individual]
    thisemail = studentids[individual]
    thisname = emaildb[thisemail]
    # get list of student's wrong answers
    thisquestionlist = thisstudentcorrect.viewkeys()

    remainingabs = 3 # number of allowed absences
    totalgotten = 0
    totalseen = 0
    print thisname, thisstudentcorrect
    print thisstudentcorrect.viewitems()
    for thisday in range (1,day):
#        print day, remainingabs
        # is thisday actually entered in the thisstudentcorrect DB?  
        # if not, it's an absence.  Increment absence counter.
        if (thisday not in thisstudentcorrect.viewkeys()):
#            print "not here day", thisday, "seen ", totalseen, " remaining abs are ",remainingabs
            if (remainingabs <=0): # if absence-counter has run over, student gets questions wrong
                totalseen += questionstoday[thisday];
#                print "penalized ", questionstoday[thisday]
            remainingabs -= 1;

        # if student is present, add both gotten-right and number-seen to total
        else:
            totalgotten += thisstudentcorrect[thisday];
            totalseen += questionstoday[thisday];

    mywriter.writerow([thisname,individual,thisemail,totalgotten,totalseen,remainingabs])
    print thisname, ",",totalgotten, ",",totalseen, " rab = ",remainingabs
